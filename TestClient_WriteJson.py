...
# Thêm 79 người nữa để đủ 100
for i in range(21, 100):
    id = f"C{i}"
    full_name = f"Nguyễn Văn Test{i}" if i % 2 == 0 else f"Trần Thị Test{i}"
    gender = "Nam" if i % 2 == 0 else "Nữ"
    username = f"user{i}"
    password = "123456"
    type_of_pt = "Newbie" if i % 3 == 0 else "Experienced"
    branch = "Hà Nội" if i % 2 == 0 else "Hồ Chí Minh"
    email = f"user{i}@gmail.com"
    phone = f"09{i % 10}{i:07d}"
    schedule = f"S{i % 5 + 1}"
    regis_id = f"R{i % 6 + 1}" if i % 4 != 0 else None
    remaining_days = i * 10 if i % 2 == 0 else i * 5
    clients.append(Client(id, full_name, gender, username, password, type_of_pt, branch, email, phone, schedule, regis_id, remaining_days))
